...
if not unitel_data.empty:
    ...
    fee_collected_percentage = (
        unitel_data['total_collected_fee'].sum() / unitel_data['total_fee_estimate'].sum()
    ) * 100
    fee_remaining = unitel_data['total_fee_estimate'].sum() - unitel_data['total_collected_fee'].sum()
    ...
    fee_col2.markdown(
        ...
        """.format(unitel_data['total_collected_fee'].sum(), fee_collected_percentage),
        unsafe_allow_html=True,
    )
