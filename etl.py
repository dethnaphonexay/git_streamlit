...
etl_data = data[data['operator_name'] == 'ETL']
...
if not etl_data.empty:
    ...
    cols[3].markdown(
        ...
        """.format(etl_data['active_mbb'].sum()),
        unsafe_allow_html=True,
    )
    cols[4].markdown(
        """
        ...
        """.format(etl_data['active_fbb'].sum()),
        unsafe_allow_html=True,
    )
    cols[5].markdown(
        """
        ...
        """.format(etl_data['disable_mbb'].sum()),
        unsafe_allow_html=True,
    )
    ...
    service_col1.metric("Total Fee Estimate MBB (LAK)", f"{etl_data['total_fee_charge_mbb'].sum():,}")
    service_col2.metric("Total Fee Collected MBB (LAK)", f"{etl_data['total_collected_fee_mbb'].sum():,}")
    service_col3.metric("Total Fee Estimate FBB (LAK)", f"{etl_data['total_fee_charge_fbb'].sum():,}")
    service_col4.metric("Total Fee Collected FBB (LAK)", f"{etl_data['total_collected_fee_fbb'].sum():,}")

    # Pie Charts
    st.subheader("Visualizations (ETL)", divider="gray")
    pie_col1, pie_col2, pie_col3 = st.columns(3)

    # Total fee collected vs estimated
    fig1 = px.pie(
        values=[
            etl_data['total_collected_fee'].sum(),
            etl_data['total_fee_estimate'].sum() - etl_data['total_collected_fee'].sum()
        ],
        names=["Collected", "Remaining"],
        title="Total Fee Collected (ETL)"
    )
    ...
    fig2 = px.pie(
        values=[
            etl_data['total_collected_fee_mbb'].sum(),
            etl_data['total_fee_charge_mbb'].sum() - etl_data['total_collected_fee_mbb'].sum()
        ],
        names=["Collected MBB", "Remaining MBB"],
        title="MBB Fee Collected (ETL)"
    )
    ...
    fig3 = px.pie(
        values=[
            etl_data['total_collected_fee_fbb'].sum(),
            etl_data['total_fee_charge_fbb'].sum() - etl_data['total_collected_fee_fbb'].sum()
        ],
        names=["Collected FBB", "Remaining FBB"],
        title="FBB Fee Collected (ETL)"
    )
    pie_col3.plotly_chart(fig3)
else:
    st.warning("No data available for ETL.")
