...
if not data.empty:
    ...
    cols[5].markdown(
        ...
        """.format(tplus_data['disable_fbb'].sum()),
        unsafe_allow_html=True,
    )
...
# โหลดข้อมูลไฟล์ CSV
file_path_sub = "total_sub_data_tplus.csv"  # ระบุไฟล์ CSV ของคุณ
data_sub = load_data(file_path_sub)

file_path_fee = "total_daily_fee_tplus.csv"  # ระบุไฟล์ CSV ของคุณ
